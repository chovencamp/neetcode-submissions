...
def get_values(hash_map: Dict[str, int], keys: List[str]) -> List[int]:
    ...
    #loop on keys 
    for key in keys:
        #we are putting the key at that hashmap and appending its value. Based on instructions we can assume all the keys in the list are in the hasmap we passed in 
